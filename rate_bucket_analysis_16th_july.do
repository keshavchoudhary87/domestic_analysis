...
cd ..
cd "raw_files"
import excel "rates_assigning_weights.xlsx", sheet("Sheet4") firstrow clear
cd ..
cd "intermediate_files"
save "rates_assigning_weights.dta", replace

* Basic cleaning/checks on weighted rates file
* Generating temporary file containing split number of each HSN
use "rates_assigning_weights.dta", clear
...
gen dummy = 1
gen digit = strlen(hsn_4)
bysort hsn_4: egen splits = total(dummy)
drop dummy
...
* Splitting at 2 digit level
forvalues x = 1/23 { 
use "rates_analysis_master_file_all_rates_m`x'.dta", clear
rename chapter_new hsn_2 
merge m:1 hsn_2 using "split_tempfile_2_digit.dta" 
keep if _merge==3
drop _merge
drop cess_cr collections_cr chapter_raw desc 
bysort hsn_2: asgen cash_ratio_new = cash_ratio_hsn, weights(liab_cr)
...
replace event =2 if (month >=6 & month<=12)
replace event =3 if (month>=14 & month <=18)
replace event = 4 if (month >=20 & month <=21)
